...
def load_data():
    ...
    try:
        data['campaigns'] = pd.read_csv(f"{data_path}campaign_performance.csv")
        data['campaigns']['date'] = pd.to_datetime(data['campaigns']['date'], format='%d/%m/%Y')
...
def page_executive_overview(data):
    ...
    # Aggregate by month
    monthly_revenue = campaigns.groupby(pd.Grouper(key='date', freq='ME'))['revenue'].sum().reset_index()
